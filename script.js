...
document.getElementById("reportForm").addEventListener("submit", async (e) => {
    ...
    showSpinner(true);
    try {
        const res = await fetch("https://city-guardian.onrender.com/send-report", { method: "POST", body: formData });
        const data = await res.json();
...
// ---------------- GLOBAL VARIABLES ----------------
let latitude = null, longitude = null;
let addressText = "Fetching location..."; // Stores address for the email signature
let map, marker, lightTiles, darkTiles;
...
navigator.geolocation.getCurrentPosition(async (pos) => {
    ...
    // Fetch and store address globally so it's ready for submission
    addressText = await reverseGeocode(latitude, longitude);
    
    document.getElementById("locationInfo").innerText = addressText;
    document.getElementById("locationSkeleton").classList.add("d-none");
    document.getElementById("mapSkeleton").classList.add("d-none");
    document.getElementById("locationInfo").classList.remove("d-none");
    document.getElementById("map").style.display = "block";
    ...
    document.getElementById("locationInfo").innerText = "Location permission denied";
    showToast("Please enable GPS for accurate reporting", "warning");
});
...
async function reverseGeocode(lat, lng) {
    try {
        const res = await fetch(`https://nominatim.openstreetmap.org/reverse?format=json&lat=${lat}&lon=${lng}`);
        const data = await res.json();
        return data.display_name || `Lat: ${lat}, Lon: ${lng}`;
    } catch (err) {
...
function generatePDF(name, complaint, dept, address) {
    const { jsPDF } = window.jspdf;
    const doc = new jsPDF();
    doc.setFont("helvetica", "bold");
    doc.text("CITYGUARDIAN REPORT RECEIPT", 20, 20);
    ...
    doc.text(doc.splitTextToSize(complaint, 160), 20, 95);
    doc.save(`Receipt_${Date.now()}.pdf`);
}
...
document.getElementById("reportForm").addEventListener("submit", async (e) => {
    ...
    formData.append("latitude", latitude);
    formData.append("longitude", longitude);
    formData.append("address", addressText); // Critical for the email signature
    ...
    try {
        ...
        if (res.ok && data.status === "success") {
            showToast("Success! Downloading Receipt...", "success");
            saveToHistory({ complaint: complaintVal, department: data.department, urgency: data.urgency });
            generatePDF(nameVal, complaintVal, data.department, addressText);
            
            document.getElementById("reportForm").reset();
            document.getElementById("imagePreview").classList.add("d-none");
            showToast("Your report is being processed.", "info");
        } 
